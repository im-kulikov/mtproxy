...
`docker run -d -p443:443 --name=mtproxy --restart=always -v ./config:/data imkulikov/mtproxy:latest`
...
`docker run -d -p443:443 -v ./config:/data -e SECRET=00baadf00d15abad1deaa51sbaadcafe imkulikov/mtproxy:latest`
...
`docker run -d -p443:443 -v ./config:/data -e SECRET_COUNT=4 imkulikov/mtproxy:latest`

A custom advertisement tag may be provided using the TAG environment variable:
`docker run -d -p443:443 -v ./config:/data -e TAG=3f40462915a3e6026a4d790127b95ded imkulikov/mtproxy:latest`.
...
`docker run -d -p443:443 -v ./config:/data -e WORKERS=16 imkulikov/mtproxy:latest`
...
## Patches
- Randr compat (from original repo)
- Fix hardcoded epoll timeout values ([PR #202](https://github.com/TelegramMessenger/MTProxy/pull/202))